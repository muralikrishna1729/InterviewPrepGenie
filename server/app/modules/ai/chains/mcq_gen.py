...
Each question needs exactly 4 options and a correct_index (0-3).
Return valid JSON matching the required structure exactly."""

    messages = [{"role": "system", "content": system_prompt}]
    logger.info("Generating MCQs: job_title=%s", job_title)
    result = await generate_structured(
        messages, response_model=McqQuestionList, model=MODEL_STANDARD
    )
